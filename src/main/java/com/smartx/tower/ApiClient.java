...
import com.smartx.tower.auth.Authentication;
...
public class ApiClient {
    ...
    private static final String DEFAULT_API_BASE_PATH = "/v2/api";
    private static final String DEFAULT_ACTIVE_PASSIVE_PROBE_PATH = "/api/healthz";

    private String rootPath = "http://localhost";
    private String apiBasePath = DEFAULT_API_BASE_PATH;
    private String probePath = DEFAULT_ACTIVE_PASSIVE_PROBE_PATH;
    ...
    private void init() {
        ...
        setUserAgent("OpenAPI-Generator/2.23.0/java");

        authentications = new HashMap<String, Authentication>();
    }

    /** Get root path. Root path may include a gateway or deployment prefix path. */
    public String getRootPath() {
        return rootPath;
    }

    /** Set root path. Root path may include a gateway or deployment prefix path. */
    public ApiClient setRootPath(String rootPath) {
        this.rootPath = trimTrailingSlash(rootPath);
        return this;
    }

    /** Get API base path appended to root path for normal SDK requests. */
    public String getApiBasePath() {
        return apiBasePath;
    }

    /** Set API base path appended to root path for normal SDK requests. */
    public ApiClient setApiBasePath(String apiBasePath) {
        this.apiBasePath = normalizePath(apiBasePath);
        return this;
    }

    /** Get active-passive probe path appended to root path for probe requests. */
    public String getProbePath() {
        return probePath;
    }

    /** Set active-passive probe path appended to root path for probe requests. */
    public ApiClient setProbePath(String probePath) {
        this.probePath = normalizePath(probePath);
        return this;
    }
     ...
     *
     * @return Base path
     * @deprecated Use {@link #getRootPath()} and {@link #getApiBasePath()}.
     */
    @Deprecated
    public String getBasePath() {
        return joinUrlPath(rootPath, apiBasePath);
    }
     ...
     * @deprecated Use {@link #setRootPath(String)} and {@link #setApiBasePath(String)}.
     */
    @Deprecated
    public ApiClient setBasePath(String basePath) {
        BasePathParts parts = splitBasePath(basePath);
        this.rootPath = parts.rootPath;
        this.apiBasePath = parts.apiBasePath;
        return this;
    }
    ...
    public boolean probeActivePassive() throws ApiException {
        return probeActivePassive(rootPath, probePath, DEFAULT_ACTIVE_PASSIVE_PROBE_TIMEOUT_MILLIS);
    }

    protected boolean probeActivePassive(String rootPath, String probePath, int timeoutMillis)
            throws ApiException {
        HttpUrl endpoint = HttpUrl.parse(rootPath);
        if (endpoint == null) {
            throw new ApiException("Invalid root path: " + rootPath);
        }

        HttpUrl probeUrl =
                endpoint.newBuilder()
                        .encodedPath(joinEncodedPath(endpoint.encodedPath(), probePath))
                        .query(null)
                        .fragment(null)
                        .build();
        Request request =
    ...
    private static BasePathParts splitBasePath(String basePath) {
        if (basePath == null || basePath.trim().isEmpty()) {
            return new BasePathParts("http://localhost", DEFAULT_API_BASE_PATH);
        }

        String raw = basePath.trim();
        HttpUrl parsed = HttpUrl.parse(raw);
        if (parsed == null) {
            return new BasePathParts(trimTrailingSlash(raw), "");
        }

        HttpUrl root = parsed.newBuilder().encodedPath("/").query(null).fragment(null).build();
        return new BasePathParts(
                trimTrailingSlash(root.toString()), normalizePath(parsed.encodedPath()));
    }
    ...
    protected static String joinUrlPath(String rootPath, String path) {
        String root = trimTrailingSlash(rootPath);
    ...
    protected static String joinEncodedPath(String rootPath, String path) {
    ...
    private static class BasePathParts {
        private final String rootPath;
        private final String apiBasePath;

        private BasePathParts(String rootPath, String apiBasePath) {
