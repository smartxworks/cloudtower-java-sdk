...
public class ITIscsiLun extends ITBase {
    ...
    public void createAndUpdateAndDeleteIscsiLun()
            throws IllegalAccessException, IllegalArgumentException, InvocationTargetException,
        ...
        try {
            ...
            IscsiLunUpdationParams updateParams =
                    new IscsiLunUpdationParams()
                            .where(new IscsiLunWhereInput().id(lun.getId()))
                            .data(
                                    new IscsiLunUpdationParamsData()
                                            .allowedInitiators(lun.getAllowedInitiators()));
            List<WithTaskIscsiLun> updateResult = api.updateIscsiLun(updateParams);
            waitForTaskSucceed(updateResult.get(0).getTaskId());
            assertThat(updateResult).as("check result of updateIscsiLun").isNotNull();
            IscsiLunDeletionParams deletionParams =
                    new IscsiLunDeletionParams()
                            .where(new IscsiLunWhereInput().id(lun.getId()))
                            .data(new IscsiLunDeletionParamsData().removeSnapshot(true));
            List<WithTaskDeleteIscsiLun> deleteResult = api.deleteIscsiLun(deletionParams);
    ...
    public void getIscsiLuns(String payload) {
        try {
            // parse params from json payload
            GetIscsiLunsRequestBody params =
                    gson.fromJson(payload, new TypeToken<GetIscsiLunsRequestBody>() {}.getType());
            // do some modify to params(optional)
            List<IscsiLun> result = api.getIscsiLuns(params);
    ...
    public void getIscsiLunsConnection(String payload) {
        try {
            // parse params from json payload
            GetIscsiLunsConnectionRequestBody params =
                    gson.fromJson(
                            payload,
                            new TypeToken<GetIscsiLunsConnectionRequestBody>() {}.getType());
            // do some modify to params(optional)
            IscsiLunConnection result = api.getIscsiLunsConnection(params);
