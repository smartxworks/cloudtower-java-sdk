...
public class ITVmVolume extends ITBase {
    ...
    public void createAndDeleteVmVolume(String payload) {
        try {
            ...
            api.deleteVmVolumeFromVm(
                    new VmVolumeDeletionParams()
                            .where(new VmVolumeWhereInput().id(volume.getId())));
            try {
                Thread.sleep(1000);
            } catch (InterruptedException e) {
            }
    ...
    public void getVmVolumes(String payload) {
        try {
            // parse params from json payload
            GetVmVolumesRequestBody params =
                    gson.fromJson(payload, new TypeToken<GetVmVolumesRequestBody>() {}.getType());
            // do some modify to params(optional)
    ...
    public void getVmVolumesConnection(String payload) {
        try {
            // parse params from json payload
            GetVmVolumesConnectionRequestBody params =
                    gson.fromJson(
                            payload,
                            new TypeToken<GetVmVolumesConnectionRequestBody>() {}.getType());
            // do some modify to params(optional)
