...
public class ITBase {
    ...
    public void getClient() throws Exception {
        ...
        if (fixtureStream == null) {
            ...
            fixtureData =
                    gson.fromJson(
                            ITUtils.readInputStream(fixtureStream),
            ...
            case "defaultCluster":
                result =
                        new ClusterApi(client)
                                .getClusters(getFixtureData(dataType, GetClustersRequestBody.class))
                                .get(0);
                break;
            case "firstAlertNotEnded":
                result =
                        new AlertApi(client)
                                .getAlerts(getFixtureData(dataType, GetAlertsRequestBody.class))
                                .get(0);
                break;
            case "firstGlobalAlertRule":
                result =
                        new GlobalAlertRuleApi(client)
                                .getGlobalAlertRules(
                                        getFixtureData(
                                                dataType, GetGlobalAlertRulesRequestBody.class))
                                .get(0);
                break;
            case "defaultVlan":
                result =
                        new VlanApi(client)
                                .getVlans(getFixtureData(dataType, GetVlansRequestBody.class))
                                .get(0);
        }
        if (result == null) {
    ...
    protected <TArgs, TApi, TResource> Object waitForResourceCreation(
            TArgs args,
            TApi api,
            String func,
            Class<TResource> classOfTResource,
            Class<TArgs> classOfTArgs)
    ...
    protected <TArgs, TApi, TResource> Object waitForResourceCreation(
        ...
        TResource resource =
                (TResource)
                        api.getClass()
                                .getDeclaredMethod(func, classOfTArgs, String.class)
                                .invoke(api, args);
    ...
    protected <TArgs, TApi, TResource> Object waitForResourceAsyncStatus(
            TArgs args,
            TApi api,
            String func,
            Class<TResource> classOfTResource,
        ...
        if (resource instanceof List) {
            ...
            while (first != null
                    && first.getClass().getDeclaredMethod("getEntityAsyncStatus").invoke(first)
                            != null) {
                if (System.currentTimeMillis() - start > TIMEOUT) {
                    throw new ApiException(408, "Timeout while waiting for async status");
                }
            ...
            while (resource != null
                    && classOfTResource
                                    .getClass()
                                    .getDeclaredMethod("getEntityAsyncStatus")
                                    .invoke(resource)
                            != null) {
                if (System.currentTimeMillis() - start > TIMEOUT) {
                    throw new ApiException(408, "Timeout while waiting for async status");
    ...
    protected <TArgs, TApi, TResource> void waitForResourceDeletion(
            TArgs args,
            TApi api,
            String func,
            Class<TResource> classOfTResource,
        ...
        if (resource instanceof List) {
            ...
            while (resourceList.size() > 0) {
                ...
                Object first = resourceList.get(0);
                if (first.getClass().getDeclaredMethod("getEntityAsyncStatus").invoke(first)
                        != EntityAsyncStatus.DELETING) {
    ...
    protected <TArgs, TApi, TResource> Object waitForResourceUpdationToTargetValue(
            TArgs args,
            TApi api,
            Method method,
            Class<TResource> classOfTResource,
    ...
    protected void waitForTaskSucceed(String taskId) throws ApiException {
        ...
        Task task =
                taskApi.getTasks(new GetTasksRequestBody().where(new TaskWhereInput().id(taskId)))
                        .get(0);
        while (task.getStatus().equals(TaskStatus.EXECUTING)
            ...
            task =
                    taskApi.getTasks(
                                    new GetTasksRequestBody()
                                            .where(new TaskWhereInput().id(taskId)))
                            .get(0);
        }
