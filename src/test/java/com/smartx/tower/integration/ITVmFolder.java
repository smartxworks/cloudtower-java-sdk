...
public class ITVmFolder extends ITBase {
    ...
    public void deleteVmFolder() throws InterruptedException, ApiException {
        Thread.sleep(1000);
        api.deleteVmFolder(
                new VmFolderDeletionParams().where(new VmFolderWhereInput().id(folder.getId())));
        folder = null;
        Thread.sleep(1000);
    }
    ...
    public void getVmFolders(String payload) {
        try {
            // parse params from json payload
            GetVmFoldersRequestBody params =
                    gson.fromJson(payload, new TypeToken<GetVmFoldersRequestBody>() {}.getType());
            // do some modify to params(optional)
    ...
    public void getVmFoldersConnection(String payload) {
        try {
            // parse params from json payload
            GetVmFoldersConnectionRequestBody params =
                    gson.fromJson(
                            payload,
                            new TypeToken<GetVmFoldersConnectionRequestBody>() {}.getType());
            // do some modify to params(optional)
    ...
    @Test(
            dataProvider = "vmFolderPayload",
            groups = {"need_vm_folder"})
    public void updateVmFolder(String payload) {
        try {
            // parse params from json payload
            VmFolderUpdationParams params =
                    gson.fromJson(payload, new TypeToken<VmFolderUpdationParams>() {}.getType());
            params.data(
                            new VmFolderUpdationParamsData()
                                    .name(
                                            "tower-sdk-test-vm-folder-update"
    ...
    public void addAndRemoveVmToFolder()
            throws IllegalAccessException, IllegalArgumentException, InvocationTargetException,
        ...
        try {
            ...
            // do some modify to params(optional)
            params.where(new VmWhereInput().id(vm.getId()))
                    .data(new VmAddFolderParamsData().folderId(folder.getId()));
            List<WithTaskVm> result = vmApi.addVmToFolder(params);
            waitForTaskSucceed(result.get(0).getTaskId());
            waitForTaskSucceed(
                    vmApi.removeVmToFolder(
                                    new VmOperateParams().where(new VmWhereInput().id(vm.getId())))
                            .get(0)
                            .getTaskId());
